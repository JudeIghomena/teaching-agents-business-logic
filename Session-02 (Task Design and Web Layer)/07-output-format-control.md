...
The validators and parsers are generated by Claude Code and added directly
into the route files where they are used. There are no separate validator files.

```
starter-code/
├── CLAUDE.md           Output Formats section lists the exact specs for all
│                       three agents - Claude Code reads these before generating
│                       the validators to ensure they match the documented format
├── package.json        Node dependencies
├── requirements.txt    Python dependencies
└── .env.example        Environment variable reference
```

The Output Formats section of your CLAUDE.md is the specification. The
validators Claude Code generates must match it exactly. If they do not,
paste the CLAUDE.md Output Formats section into Claude Code and ask it to fix.
