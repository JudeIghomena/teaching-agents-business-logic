...
The output of this document is `agent/prompts/matteo_v1.txt` - a text file
generated by Claude Code using the prompt above. No pre-written code is provided
because the system prompt is specific to your agent's task definition.

```
starter-code/
├── CLAUDE.md           Context for Claude Code including your Task Definitions
├── package.json        Node dependencies
├── requirements.txt    Python dependencies (anthropic - needed for the token counter)
└── .env.example        ANTHROPIC_API_KEY required to run the token counter
```

After Claude Code generates the prompt file, the token counter command in the
Claude Code Desktop App section above verifies it fits within the budget from
Framework 04.
