...
The iteration tooling is generated by Claude Code from the prompt above.
By this point in Session 02, your project already has the evaluation suite
from document 08 and the prompt files from documents 05 and 06.

```
starter-code/
├── CLAUDE.md           Pre-Commit Gate section defines what must pass before
│                       any prompt file is committed - Claude Code enforces this
├── package.json        Node dependencies
├── requirements.txt    Python dependencies
└── .env.example        Environment variable reference
```

Claude Code generates the version-aware loader in `agent/context.py`,
the `measure_prompt.py` token counter, and the blank `PROMPT_ITERATIONS.md`
log. Your existing `run_golden.py` from document 08 is reused as-is.
